...
To run the smoke tests against a running instance:
...
src/
  main/
    ...
    resources/
      ...
      db/backup/       # Data-only SQL backup files generated by just db-backup
...
|---|---|---|---|
| `alice.admin` | `alice.admin@example.test` | `ADMIN` | `S3cure!pw` |
| `grace.lead` | `grace.lead@example.test` | `GROUP_LEAD` | `S3cure!pw` |
| `sam.member` | `sam.member@example.test` | `MEMBER` | `S3cure!pw` |

The script creates 9 prompts total: 3 authored by each seeded user in the shared group. This exercises the existing relationship from `prompts.created_by_member_id` to `members.id`, plus the role relationship through `group_memberships`.

---

## Backup and restore

Use `just db-backup` to export all current table data as data-only SQL files under `src/main/resources/db/backup/`.

```bash
just db-backup
```

The backup files are numbered for foreign-key-safe restore order:

| File | Table |
|---|---|
| `01-organizations.sql` | `organizations` |
| `02-members.sql` | `members` |
| `03-groups.sql` | `groups` |
| `04-group-memberships.sql` | `group_memberships` |
| `05-prompts.sql` | `prompts` |

The export uses `mariadb-dump` from the running database container with data-only, complete-column `INSERT` statements. Schema DDL is not included because Flyway owns schema creation.

To restore the generated backup files into a migrated database, run:

```bash
just db-restore-backup
```

Restore expects the database container to be running and the schema to already exist. The generated SQL uses plain `INSERT` statements, so restore is intended for an empty or non-conflicting database.

For a clean restore from scratch:
...
| Recipe | Description |
|---|---|
| `just ci` | Full lifecycle: stop app, rebuild the DB, run the app verification gate, start the app, then run HTTP and smoke checks |
| `just db-start` | Start the MariaDB container and then run the application |
...
| `just db-backup` | Export current table data to numbered SQL files under `src/main/resources/db/backup/` |
| `just db-backup-restore` | Restore numbered backup SQL files into a migrated database |
| `just db-rebuild` | Destroy, recreate, migrate, and seed the local database |
| `just build` | Compile sources only |
...
| `just smoke` | Run the end-to-end smoke test against `localhost:8080` |
...
Enforces a minimum 80% line coverage and 80% branch coverage across all non-excluded classes. The JaCoCo report is generated at `target/site/jacoco/index.html` after every `just test-unit` run.
...
```bash
# Prerequisites: application running, jq installed
just db-start      # in one terminal
just smoke         # in another terminal
```

The smoke test creates a fresh organization, primary member, group, and secondary member on every run (using a timestamp suffix to avoid collisions), assigns the secondary member a selected group role, exercises ADMIN organization list/rename/delete-conflict behavior, exercises the complete prompt lifecycle (create → list → get → update → delete), then tears down the group. It exits non-zero on the first failure and prints which step failed.

Expected output:

```
==> smoke: create organization
  ✓ POST /api/organizations  →  <uuid>

==> smoke: register member
  ✓ POST /api/members  →  <uuid>

==> smoke: register member with selected group role
  ✓ POST /api/members + POST /api/groups/<uuid>/members  →  GROUP_LEAD
...
Runs every `.http` file in `src/test/resources/http` through `httpyac` with the `dev` environment from `src/test/resources/http/http-client.env.json`.
...
Runs the full local lifecycle gate: stops any app on `localhost:8080`, destroys and recreates the database volume, applies Flyway migrations, seeds sample data, runs `just app-verify`, starts the packaged JAR in the background, waits for HTTP readiness, runs `just test-http`, runs `just smoke`, and stops the app on exit.
